...
# def get_input_batch_cornet(device, filename):
#     input_image = Image.open(filename).convert("RGB")
#     preprocess = transforms.Compose([
#         transforms.Resize(256),
#         transforms.CenterCrop(224),
...
# def run_fgsm_pipeline(model, device, filename, epsilon=0.05):
#     if 'alexnet' in model.__class__.__name__.lower():
#         input_batch = get_input_batch(device, filename)
#     elif 'cornet' in model.__class__.__name__.lower():
#         input_batch = get_input_batch_cornet(device, filename)
#     elif 'vgg' in model.__class__.__name__.lower():
#         input_batch = get_input_batch_vgg(device, filename)
