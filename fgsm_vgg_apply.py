...
for filename in all_images:
    try:
        ...
        if is_correct_before == True: # if it passes vgg
        ...
        for eps in epsilons:
            ...
            try:
                save_adv_image(perturbed_image, eps, true_label, true_index, pred_before, pred_after, output_dir=f"adv_VGGoutputs6/adv_outputs6_eps{eps}")
...
#             perturbed_image = fgsm_attack(input_batch, eps, data_grad)
#             try:
#                 save_adv_image(perturbed_image, eps, true_label, true_index, pred_before, pred_after, output_dir=f"adv_VGGoutputs6/adv_VGGoutputs6_eps{eps}")
#                 total_per_eps[eps] += 1 # TRIAL 3: counting here how many runs are we getting with each eps
#             except Exception as e:
#                 print(f"❌ Failed to save image for {filename} at eps={eps}: {e}")
#             # log or compare accuracy
