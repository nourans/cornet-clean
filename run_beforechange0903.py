...
from run_vgg import extract_true_label
import torch
...
def test(layer='decoder', sublayer='avgpool', time_step=0, imsize=224):
    ...
    for ext in ["*.JPEG", "*.jpeg", "*.JPG", "*.jpg", "*.png"]:
        fnames.extend(glob.glob(os.path.join(FLAGS.data_path, ext)))
    ...
    with torch.no_grad():
        for image_path in tqdm.tqdm(fnames, desc="Testing Images"):
            ...
            #CHECKPOINT:
            print(f"\n🔍 CORnet Predicted Index: {predicted_index.item()}")
            ...
            basename = os.path.basename(image_path)  # e.g., '0.005_knot616_init616_adv616_23d77fcb.png'
            true_label = extract_true_label(basename)
        ...
        # end of name extraction
            ...
            print(f"{basename} → True: {true_label} | CORnet: {predicted_label}")
            #print(f"{os.path.basename(image_path)} → True: {true_label} | CORnet: {predicted_label}")
    
    length = len(fnmatch.filter(os.listdir(FLAGS.data_path), '*.*'))
